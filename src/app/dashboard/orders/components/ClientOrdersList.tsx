...
// 'user' prop mein pura database object pass karein
export default function ClientOrdersList({ initialOrders, user }: { initialOrders: any[], user: any }) {
  const [orders] = useState(initialOrders);
  const [isProcessingPayment, setIsProcessingPayment] = useState<string | null>(null);
  const [useWallet, setUseWallet] = useState(false);

  // Shaurya Kumar Singh ki ID ke hisab se balance yahan extract hoga
  // Agar balance 0 hai, toh default 0 hi lega
  ...
  return (
    <div className="space-y-12">
      {orders.map((order: any) => (
        <div key={order._id} className="bg-white rounded-[3.5rem] p-3 shadow-2xl shadow-slate-200/50 border border-slate-100 transition-all hover:border-blue-200">
          <div className="flex flex-col xl:flex-row gap-6">
            ...
            <div className="xl:w-[35%] p-2">
              <OrderBillingCard 
                order={order}
                userBalance={currentBalance} // Ab yahan 0 pass hoga
                useWallet={useWallet}
                setUseWallet={setUseWallet}
                isProcessingPayment={isProcessingPayment === order._id}
                onPay={handlePayment}
              />
            </div>
