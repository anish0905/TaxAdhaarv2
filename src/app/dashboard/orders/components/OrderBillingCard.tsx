...
import { 
  ...
  Zap, QrCode, ShieldCheck, ChevronDown, ChevronUp, Clock 
} from "lucide-react";
...
export function OrderBillingCard({ order, userBalance, useWallet, setUseWallet }: any) {
  ...
  // --- Logic for Payment Visibility ---
  const isReadyForPayment = order.status === "payment_pending";
  ...
  return (
    <div className="bg-[#020617] text-white rounded-[2.5rem] p-6 h-fit sticky top-24 shadow-2xl border border-white/5 overflow-hidden">
      ...
      {/* 2. WALLET SECTION */}
      <div 
        onClick={() => isReadyForPayment && userBalance > 0 && setUseWallet(!useWallet)}
        ...
        } ${(!isReadyForPayment || userBalance === 0) ? 'opacity-30 grayscale cursor-not-allowed' : 'active:scale-95'}`}
      >
      ...
      {/* 4. DYNAMIC PAYMENT GATEWAY SECTION */}
      <div className="space-y-3">
        <p className="text-[8px] font-black text-slate-600 uppercase text-center tracking-[0.2em]">
          {isReadyForPayment ? "Select Gateway" : "Order Status"}
        </p>
        ...
        {isReadyForPayment ? (
          ...
          <div className="grid grid-cols-1 gap-3 animate-in fade-in duration-500">
            <div className="w-full">
              <RazorpayButton orderId={order._id.toString()} useWallet={useWallet} />
            </div>
          ...
          // --- UNDER REVIEW STATE ---
          <div className="bg-blue-600/10 border border-blue-500/20 rounded-[1.5rem] p-6 text-center animate-in zoom-in-95 duration-500">
            <Clock className="mx-auto text-blue-500 mb-3 animate-pulse" size={28} />
            <h5 className="text-[10px] font-black uppercase text-blue-400 tracking-widest mb-1">Documents Under Review</h5>
            <p className="text-[8px] text-slate-500 font-bold leading-relaxed italic">
              Aapke documents verify ho rahe hain. <br/> Verification ke baad payment link enable hoga.
            </p>
          </div>
        ) : (
          // --- OTHER STATES (e.g. Processing/Completed) ---
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-[1.5rem] p-6 text-center">
             <CheckCircle2 className="mx-auto text-emerald-500 mb-2" size={24} />
             <p className="text-[10px] font-black uppercase text-emerald-400 tracking-widest">Payment Completed</p>
          </div>
        )}
      </div>
