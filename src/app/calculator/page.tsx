...
import { Download, ShieldCheck, TrendingUp, X, Phone, User as UserIcon, CreditCard, ArrowUpRight, Activity, Percent, ArrowRight, CheckCircle2 } from "lucide-react";
...
export default function TaxCalculatorPage() {
  ...
  return (
    <>
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-24 antialiased text-slate-950 selection:bg-blue-600 selection:text-white tracking-tight">
      <PublicNavbar />
      
      {/* Premium Hero Banner Background Mesh */}
      ...
      <div className="max-w-5xl mx-auto px-6 mt-16 space-y-10">
        ...
        {/* 🏛️ HEADER SECTION (Micro-text & Premium Spacing) */}
        <div className="flex flex-col items-center text-center space-y-3 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="relative">
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter max-w-xl leading-[1.1]">
        ...
        {/* 🛠️ MAIN ARCHITECTURE GRID */}
        <div className="grid lg:grid-cols-12 gap-6 items-start pt-2">
          
          {/* CONTROLS PANEL (Left Side - Compact UI) */}
          <div className="lg:col-span-4 bg-white p-6 rounded-[2rem] shadow-xl shadow-slate-200/30 border border-slate-200/60 space-y-8 relative overflow-hidden backdrop-blur-md">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-end">
                ...
                <span className="text-xl font-black text-slate-900 tracking-tighter">₹{formatCurr(salary)}</span>
              </div>
              <div className="relative">
                <input type="range" min="300000" max="5000000" step="50000" value={salary} onChange={(e)=>setSalary(Number(e.target.value))} className="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-blue-600 focus:outline-none transition-all" />
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-end">
                ...
                <span className="text-xl font-black text-emerald-600 tracking-tighter">₹{formatCurr(inv)}</span>
              </div>
              <div className="relative">
                <input type="range" min="0" max="250000" step="5000" value={inv} onChange={(e)=>setInv(Number(e.target.value))} className="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-emerald-500 focus:outline-none transition-all" />
                <div className="flex justify-between text-[8px] font-black text-slate-300 uppercase tracking-widest pt-1">
                  <span>0</span>
                  <span>1.5L</span>
                  <span>2.5L</span>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* Savings Meter Badge Block */}
            <SavingsMeter oldTax={results.oldTax} newTax={results.newTax} />

            {/* 🔥 HIGH-CONVERSION: Dynamic Financial Utilities Dashboard */}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-1">
               <button 
                ...
                className="flex-1 bg-slate-900 text-white px-6 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2 active:scale-95 shadow-sm shadow-slate-900/5">
                 <Download size={12} /> Download Audit Report
               </button>
               <Link href="/services" className="flex-1 bg-blue-600 text-white px-6 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-md shadow-blue-500/10 flex items-center justify-center gap-1.5 group">
        ...
        {/* 🏛️ OFFICIAL CORPORATE FOOTNOTE */}
        <div className="pt-8 text-center border-t border-slate-200/40 max-w-xs mx-auto">
          <Link 
            href="https://www.aimgrit.in" 
            target="_blank" 
            className="inline-flex items-center gap-1.5 text-[8px] font-black text-slate-400 hover:text-blue-600 transition-all uppercase tracking-widest bg-white border border-slate-200 px-3 py-1.5 rounded-md shadow-sm"
          >
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm p-8 rounded-[2rem] shadow-2xl relative overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-slate-400 text-[10px] font-bold leading-normal max-w-xs mx-auto uppercase tracking-wider">Encrypted Report Secure Delivery Pipeline</p>
              ...
              <form onSubmit={finalizeDownload} className="space-y-4 pt-4 text-left">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input required type="tel" placeholder="7557721426" className="w-full pl-10 pr-4 py-3 bg-slate-50 focus:bg-white rounded-lg border border-slate-200 focus:border-blue-500 outline-none font-bold text-slate-900 transition-all text-xs" 
                      onChange={(e) => setLeadData({...leadData, phone: e.target.value})} />
                  </div>
                </div>
                <button type="submit" className="w-full bg-blue-600 text-white py-3.5 rounded-lg font-black text-[10px] uppercase tracking-widest shadow-md shadow-blue-500/10 hover:bg-blue-700 transition-all mt-2 flex items-center justify-center gap-1.5">
...
// PREMIUM INCOME SAVINGS STRIP COMPONENT
...
// CIBIL METER & AFFILIATE PORTAL GRID COMPONENT
function AffiliateOffers({ oldTax, newTax }: { oldTax: number, newTax: number }) {
  const savings = Math.abs(oldTax - newTax);
  if (savings <= 0) return null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid lg:grid-cols-12 gap-4">
        ...
        {/* 📊 LEFT: Premium Interactive CIBIL Dashboard */}
        <div className="lg:col-span-7 bg-white border border-slate-200/80 rounded-[2rem] p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
          ...
          <Link 
            href="https://sales.gromo.in/c/ItiN_CCVj1XzQrqkDxIgr" 
            target="_blank" 
            className="w-full py-3 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest text-center flex items-center justify-center gap-1.5 hover:bg-slate-800 transition-all shadow-sm active:scale-[0.99]"
          >
            Verify Bureau Score <ArrowUpRight size={12} />
          </Link>
        </div>

        {/* 💳 RIGHT: Axis Neo Credit Card Layout */}
        <div className="lg:col-span-5 bg-white border border-slate-200/80 rounded-[2rem] p-6 shadow-sm flex flex-col justify-between overflow-hidden relative">
          ...
          <Link 
            href="https://leads.banksathi.com/?h=eTBId2hhQTNab3p2Zkt2V1VZb2Rvdz09" 
            target="_blank" 
            className="w-full py-3 bg-[#8F0025] text-white rounded-xl font-black text-[10px] uppercase tracking-widest text-center flex items-center justify-center gap-1.5 hover:bg-[#70001D] transition-all shadow-md mt-4 active:scale-[0.99]"
          >
...
// sleek MINIMAL REGIME RESULT COMPONENT
